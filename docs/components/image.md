...
>增强版的 image 标签，提供多种图片填充模式，支持图片懒加载、加载中提示、加载失败提示。
...
```html
<w-image use-loading-slot>
  <template #loading>
    <w-loading type="spinner" size="20" vertical />
  </template>
</w-image>
```
...
| webp | 是否解析 webp 格式 | _boolean_ | `false` |
