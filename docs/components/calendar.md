...
```html
<template>
  <w-cell title="选择单个日期" :value="date" @click="onDisplay" />
  <w-calendar :show="show" @close="onClose" @confirm="onConfirm" />
</template>
<script setup>
import { ref } from 'vue'
const data = ref('')
...
```html
<template>
  <w-cell title="选择多个日期" :value="text" @click="onDisplay" />
  <w-calendar
    :show="show"
    type="multiple"
    @close="onClose"
...
function onConfirm(event) {
  show.value = false
  text.value = `选择了 ${event.length} 个日期`
}
</script>
```
...
```html
<template>
  <w-cell title="选择日期区间" :value="date" @click="onDisplay" />
  <w-calendar
    :show="show"
    type="range"
    @close="onClose"
...
```html
<w-calendar :show="show" :show-confirm="false" />
```
...
```html
<w-calendar :show="show" color="#07c160" />
```
...
```html
<template>
  <w-calendar :show="show" type="range" :formatter="formatter" />
</template>
<script setup>
import { ref } from 'vue'
...
```html
<w-calendar :show="show" round="false" position="right" />
```
...
```html
<w-calendar type="range" :max-range="3" />
```
...
```html
<w-calendar :first-day-of-week="1" />
```
...
> Tips: 注意，在自定义calendar的高度时，需要确保，滚动到当前月份时，所有的日期要展现在.wan-calendar__body内，否则可能会出现滚动时头部月份与当前月份不同步的情况。目前平铺型预设高度618px,弹窗型高度90%。
...
| click-subtitle | 点击日历副标题时触发 | _TouchEvent_ |
